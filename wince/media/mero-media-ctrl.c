/*
 * mero-media-ctrl.c - Native Cybernetic Companion Deck: YouTube Music Viewer & Cyberpunk Weather Terminal
...
/* --- Media State --- */
static WCHAR          g_title[128]  = L"Waiting for YouTube Music...";
...
/* Artwork buffer - 214x214 large format */
#define ART_SIZE 214
...
/* Load and scale album cover art into 214x214 32bpp DIB Section with calibrated LUT */
static void LoadCoverArt(const WCHAR *path)
{
    ...
    /* Scaling into 214x214 surface with color LUT */
    int dy, dx;
...
static void PollStorageNowPlaying(void)
{
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        ...
        /* Detect track change or check if cover file size changed */
        ...
        /* Check cover file */
        const WCHAR *coverPath = (GetFileAttributesW(COVER_FILE_SDMMC) != 0xFFFFFFFF) ? COVER_FILE_SDMMC :
                                 (GetFileAttributesW(COVER_FILE_FLASH) != 0xFFFFFFFF) ? COVER_FILE_FLASH : NULL;
        if (coverPath) {
            HANDLE hCov = CreateFileW(coverPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
            if (hCov != INVALID_HANDLE_VALUE) {
                DWORD sz = GetFileSize(hCov, NULL);
                CloseHandle(hCov);
                if (trackChanged || !g_hasCover || sz != g_lastCoverSize) {
...
/* Vector Weather Glyph Renderer (GDI) - Guaranteed Zero White Artifacts */
static void DrawWeatherGlyph(HDC hdc, int x, int y, int w, int h, const WCHAR *cond)
{
    ...
    if (wcsstr(cond, L"Rain") || wcsstr(cond, L"Shower") || wcsstr(cond, L"Drizzle")) {
        ...
        HBRUSH hCloudBr = CreateSolidBrush(RGB(20, 10, 32));
        HBRUSH hOldBr = (HBRUSH)SelectObject(hdc, hCloudBr);
...
/* Paint View 1: Lean YouTube Music Viewer with Enormous Cover Art (214x214) */
static void PaintMusicView(HDC memDC)
{
    WCHAR buf[128];

    /* Left: Giant Album Cover Art (x=10, y=30, size=214x214) */
    int artX = 10, artY = 30;
    if (g_hasCover && g_coverDC) {
        ...
        HPEN hVinylPen = CreatePen(PS_SOLID, 1, RGB(25, 35, 42));
        HPEN hOld = (HPEN)SelectObject(memDC, hVinylPen);
        int r;
        for (r = 15; r < 100; r += 12) {
            Ellipse(memDC, artX + 107 - r, artY + 107 - r, artX + 107 + r, artY + 107 + r);
        }
    ...
    /* Right Column: Metadata & Playback (x = 236 to 470, width = 234) */
    int textX = 236;

    /* Row 1: Status Badge & Digital Timer */
    RECT badgeRc = { textX, 32, textX + 98, 50 };
    COLORREF badgeBorderCol = g_isPlaying ? RGB(0, 255, 128) : RGB(255, 180, 40);
    COLORREF badgeBgCol     = g_isPlaying ? RGB(0, 32, 16)   : RGB(32, 22, 0);

    HBRUSH hBadgeBr = CreateSolidBrush(badgeBgCol);
    HBRUSH hOldBr = (HBRUSH)SelectObject(memDC, hBadgeBr);
    ...
    ExtTextOutW(memDC, textX + 110, 34, 0, NULL, buf, lstrlenW(buf), NULL);

    /* Row 2: Track Title with Smooth Marquee */
    SetTextColor(memDC, RGB(80, 140, 130));
    ExtTextOutW(memDC, textX, 58, 0, NULL, L"TRACK", 5, NULL);

    RECT titleClip = { textX, 74, 470, 102 };
    HRGN hRgn = CreateRectRgnIndirect(&titleClip);
    SelectClipRgn(memDC, hRgn);

    SetTextColor(memDC, RGB(255, 255, 255));
    ExtTextOutW(memDC, textX - g_scrollOffset, 74, 0, NULL, g_title, lstrlenW(g_title), NULL);
    ...
    /* Row 3: Artist */
    SetTextColor(memDC, RGB(80, 140, 130));
    ExtTextOutW(memDC, textX, 108, 0, NULL, L"ARTIST:", 7, NULL);
    SetTextColor(memDC, RGB(255, 215, 0));
    ExtTextOutW(memDC, textX + 55, 108, 0, NULL, g_artist, lstrlenW(g_artist), NULL);

    /* Row 4: Album */
    SetTextColor(memDC, RGB(80, 140, 130));
    ExtTextOutW(memDC, textX, 130, 0, NULL, L"ALBUM:", 6, NULL);
    SetTextColor(memDC, RGB(160, 220, 230));
    ExtTextOutW(memDC, textX + 55, 130, 0, NULL, g_album, lstrlenW(g_album), NULL);

    /* Row 5: Sleek Cyber Progress Bar */
    int progY = 162;
    int barW = 230, barH = 8;
    RECT barRc = { textX, progY, textX + barW, progY + barH };
    HBRUSH hTrackBr = CreateSolidBrush(RGB(16, 24, 30));
    FillRect(memDC, &barRc, hTrackBr);
    ...
    /* Row 6: Audio Stream Tag */
    SetTextColor(memDC, RGB(0, 130, 110));
    ExtTextOutW(memDC, textX, 182, 0, NULL, L"SOURCE: YOUTUBE MUSIC // 48 KHZ STEREO", 38, NULL);
}
...
static void PaintWeatherView(HDC memDC)
{
    ...
    /* Left Section: Core Atmosphere (x = 10 to 186) */
    int leftX = 10;

    /* Coordinates / Station Callout */
    SetTextColor(memDC, RGB(0, 140, 90));
    ExtTextOutW(memDC, leftX, 32, 0, NULL, g_wxCoord, lstrlenW(g_wxCoord), NULL);
    ...
    /* Condition Badge (Obsidian green background, NO WHITE!) */
    ...
    /* Mini Cyber Bar for Humidity (dark track, green fill, NO WHITE!) */
    ...
    /* Forecast Divider */
    {
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(20, 42, 36));
        HPEN hOld = (HPEN)SelectObject(memDC, hPen);
        MoveToEx(memDC, rightX, 114, NULL);
        LineTo(memDC, g_screenW - 10, 114);
        SelectObject(memDC, hOld);
        DeleteObject(hPen);
    }

    SetTextColor(memDC, RGB(0, 255, 160));
    ExtTextOutW(memDC, rightX, 120, 0, NULL, L"SYNOPTIC OUTLOOK // 3-DAY VECTOR", 32, NULL);

    /* 3 Forecast Cards Side-by-Side (Deep obsidian green fill, NO WHITE!) */
...
static void OnPaint(HWND hWnd)
{
    ...
    if (g_currentView == VIEW_MUSIC) {
        SetTextColor(memDC, RGB(0, 200, 160));
        ExtTextOutW(memDC, 12, 6, 0, NULL, L"MERO // AUDIO DECK", 18, NULL);
    } else {
        SetTextColor(memDC, RGB(0, 255, 160));
        ExtTextOutW(memDC, 12, 6, 0, NULL, L"CANELA, RS [BR] // METEOROLOGY", 30, NULL);
    }
    ...
    SetTextColor(memDC, RGB(0, 240, 255));
    ExtTextOutW(memDC, 220, 6, 0, NULL, buf, lstrlenW(buf), NULL);
    ...
    /* Top Divider Line */
    {
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(20, 38, 32));
        HPEN hOld = (HPEN)SelectObject(memDC, hPen);
        MoveToEx(memDC, 10, 25, NULL);
        LineTo(memDC, g_screenW - 10, 25);
        SelectObject(memDC, hOld);
        DeleteObject(hPen);
    }
    ...
    /* --- FOOTER STATUS BAR --- */
    {
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(18, 35, 30));
        HPEN hOld = (HPEN)SelectObject(memDC, hPen);
        MoveToEx(memDC, 10, 248, NULL);
        LineTo(memDC, g_screenW - 10, 248);
        SelectObject(memDC, hOld);
        DeleteObject(hPen);
    }
    ...
    if (g_currentView == VIEW_MUSIC) {
        wsprintfW(buf, L"MERO DECK // CANELA RS // TAP SCREEN TO TOGGLE WEATHER HUD");
    } else {
        wsprintfW(buf, L"MERO DECK // SYNCED %ls // TAP SCREEN TO TOGGLE NOW PLAYING", g_wxUpdated);
    }
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        /* Create 214x214 32bpp DIB Section for Artwork */
        {
            HDC hdc = GetDC(hWnd);
            BITMAPINFO bmi;
        ...
        /* Timers: Poll (500ms), Marquee (150ms), Clock (1s) */
    ...
    case WM_TIMER:
        if (wParam == TIMER_ID_POLL) {
            ...
            if (g_currentView == VIEW_MUSIC && lstrlenW(g_title) > 16) {
                if (g_scrollPause > 0) {
                    g_scrollPause--;
                } else {
                    g_scrollOffset += (g_scrollDir * 2);
                    int maxScroll = (lstrlenW(g_title) * 8) - 170;
